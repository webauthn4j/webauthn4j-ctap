    ...
    override fun processCommandApdu(apdu: ByteArray, bundle: Bundle?): ByteArray? {
        launch(Dispatchers.Default) {
            ...
            if (nfcService.nfcStatus.value == NFCStatus.ON) {
                ...
                sendResponseApdu(responseAPDU.bytes)
                logger.debug("Sent response APDU: {}", ArrayUtil.toHexString(responseAPDU.bytes))
            } else {
                val errorResponseAPDU = ResponseAPDU.createErrorResponseAPDU()
                sendResponseApdu(errorResponseAPDU.bytes)
                logger.debug(
                    "Sent response APDU: {}",
                    ArrayUtil.toHexString(errorResponseAPDU.bytes)
                )
            }
        }
        return null

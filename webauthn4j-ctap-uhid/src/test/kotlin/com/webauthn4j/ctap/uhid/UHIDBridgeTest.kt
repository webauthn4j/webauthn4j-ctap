...
internal class UHIDBridgeTest {

    private class FakeUHIDConnection : UHIDConnection() {
        val writtenEvents = ConcurrentLinkedQueue<ByteArray>()
        ...
        fun enqueueEvent(event: ByteArray) { inputQueue.put(event) }

        override fun readEvent(): ByteArray {
            while (!closed) {
        ...
        override fun writeEvent(eventBytes: ByteArray) {
            writtenEvents.add(eventBytes.clone())
        }
    ...
    private class TestHIDClient(private val connection: FakeUHIDConnection) {
            ...
            for (packet in packets) {
                ...
                val event = ByteBuffer.allocate(UHIDEvent.EVENT_SIZE)
                    ...
                    .apply {
                        ...
                        put(packetBytes)
                        position(4100)
                        putShort(packetBytes.size.toShort())
                        put(0x00)
                    }
                    .array()
        ...
        fun collectResponseHIDPackets(timeoutMs: Long = 2000): List<ByteArray> {
            ...
            while (System.currentTimeMillis() < deadline) {
                ...
                val type = buf.getInt(0)
                if (type != UHIDEventType.UHID_INPUT2.value) continue
                val size = buf.getShort(4).toInt() and 0xFFFF
    ...
    fun start_createsDevice() = runBlocking {
        ...
        val bridge = UHIDBridge(authenticator, connection = fakeConnection)
        ...
        try {
            ...
            val type = ByteBuffer.wrap(createEvent!!).order(ByteOrder.LITTLE_ENDIAN).getInt(0)
            assertThat(type).isEqualTo(UHIDEventType.UHID_CREATE2.value)
        } finally {
            bridge.stop()
            scope.cancel()
        }
